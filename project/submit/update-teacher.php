...
if ($_FILES['photo']['name'] == '') {
    ...
    $sql = "UPDATE teacher SET fullname=?,mobile=?,email=?,gender=?,qualification=?,experience=?,photo=? WHERE id=?";
    $cmd = $db->prepare($sql);
    $cmd->bindParam(1,$fullname);
    $cmd->bindParam(2,$mobile);
    $cmd->bindParam(3,$email);
